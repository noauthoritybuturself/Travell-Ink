...
public function can_edit($user_id,$artist_profile_id){

	$this->db->where('user_id',$user_id);
	$this->db->where('artist_profile_id',$artist_profile_id);
    $query = $this->db->get($this->tables['users_artist_profile']);
    if ($query->num_rows() > 0){
        return true;
    }
    else{
	...
	**/
	 ...
	 * @author bruno palma
	*/
	public function create_style($style_name = FALSE,$additional_data = array())
	{
		// bail if the group name was not passed
		if(!$style_name)
		{
			$this->set_error('style_name_required');
			return FALSE;
		}

		// bail if the style name already exists
		$existing_style = $this->db->get_where($this->tables['tattoo_style'], 
			array('name' => $style_name))->num_rows();
		if($existing_style !== 0)
		{
			$this->set_error('style_already_exists');
			return FALSE;
		}

		$data = array('name'=>$style_name);

		//filter out any data passed that doesnt have a matching column in the tattoo_style table
		//and merge the set style data and the additional data
		if (!empty($additional_data)) $data = array_merge($this->_filter_data($this->tables['tattoo_style'], 
			$additional_data), $data);

		$this->trigger_events('extra_style_set');

		// insert the new style
		$this->db->insert($this->tables['tattoo_style'], $data);
		$style_id = $this->db->insert_id();

		// report success
		$this->set_message('style_creation_successful');
		// return the brand new group id
		return $style_id;
	}
	 ...
	 * @author bruno palma
	 **/
	public function update_style($style_id = FALSE, $style_name = FALSE, $additional_data = array())
	{
		...
		if (!empty($style_name))
		{
			...
			// bail if the style name already exists
			$existing_style = $this->db->get_where($this->tables['tattoo_style'], array('name' => $style_name))->row();
			if(isset($existing_style->id) && $existing_style->id != $style_id)
			{
				$this->set_error('style_already_exists');
				return FALSE;
			}
		...
		// restrict change of name of the admin style
        $style = $this->db->get_where($this->tables['tattoo_style'], array('id' => $style_id))->row();
        if($this->config->item('admin_style', 'ion_auth') === $style->name && $style_name !== $style->name)
        {
            $this->set_error('style_name_admin_not_alter');
            return FALSE;
        }
		...
		//filter out any data passed that doesnt have a matching column in the tattoo_style table
		//and merge the set style data and the additional data
		if (!empty($additional_data)) $data = array_merge($this->_filter_data($this->tables['tattoo_style'], $additional_data), $data);


		$this->db->update($this->tables['tattoo_style'], $data, array('id' => $style_id));

		$this->set_message('style_update_successful');
	...
	* @author bruno palma
	**/
	public function delete_style($style_id = FALSE)
	{
		// bail if mandatory param not set
		if(!$style_id || empty($style_id))
		{
			return FALSE;
		}
		$style = $this->style($style_id)->row();
		if($style->name == $this->config->item('admin_style', 'ion_auth'))
		{
			$this->trigger_events(array('post_delete_style', 'post_delete_style_notallowed'));
			$this->set_error('style_delete_notallowed');
			return FALSE;
		}

		$this->trigger_events('pre_delete_style');

		$this->db->trans_begin();

		// remove all profiles from this style
		$this->db->delete($this->tables['artist_profile_tattoo_style'], array($this->join['tattoo_style'] => $style_id));
		// remove the style itself
		$this->db->delete($this->tables['tattoo_style'], array('id' => $style_id));

		if ($this->db->trans_status() === FALSE)
		{
			$this->db->trans_rollback();
			$this->trigger_events(array('post_delete_style', 'post_delete_style_unsuccessful'));
			$this->set_error('style_delete_unsuccessful');
			return FALSE;
		}

		$this->db->trans_commit();

		$this->trigger_events(array('post_delete_style', 'post_delete_style_successful'));
		$this->set_message('style_delete_successful');
		return TRUE;
	}


		public function create_artist_profile($data = array())
	{



		$this->db->insert($this->tables['artist_profile'], $data);

		$artist_profile_id = $this->db->insert_id();

		// array for users_artist_profile data insertion.
		$other_array = array(
				'user_id' => $data['user_id'],
				'artist_profile_id'  => $artist_profile_id,
			);

		$this->db->insert($this->tables['users_artist_profile'], $other_array);

		//create folder for photos

		 $path = "./photos/".$artist_profile_id;
    ...
    if(!is_dir($path)) //create the folder if it's not already exists
    {
      mkdir($path,0755,TRUE);
		...
		return (isset($artist_profile_id)) ? $artist_profile_id : FALSE;
	}

	public function get_all_artist_profiles($user_id)
	{

		$this->db->where('user_id', $user_id);
		$this->response = $this->db->get($this->tables['artist_profile']);
		return $this;
	}
	...
	public function update_artist_profile($id, array $data)
	{

		$user = $this->artist_profile($id)->row();
		...
		$this->db->update($this->tables['artist_profile'], $data, array('id' => $user->id));
	...
	* @author Phil Sturgeon
	**/
	public function delete_artist_profile($id)
	{
		$this->trigger_events('pre_delete_user');
